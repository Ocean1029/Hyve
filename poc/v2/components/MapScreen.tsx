...
import { Filter, Check, MapPin, Star, Building, Clock, Heart } from 'lucide-react';
import { AppScreen, User, PlaceAggregate } from '../types';
import { Avatar } from './UI';
import { MapVisualization, Leaflet3DMarker } from './MapVisualization';

export const MapScreen = ({ buildings, onSelectBuilding, selectedBuilding, friends, isActive, focusedFriendId, isLightMode = false }: { buildings: PlaceAggregate[], onSelectBuilding: (b: PlaceAggregate | null) => void, selectedBuilding: PlaceAggregate | null, friends: User[], isActive: boolean, focusedFriendId?: string | null, isLightMode?: boolean }) => {
       ...
       <div className="absolute bottom-[88px] left-0 right-0 z-[400] pointer-events-none">
          <div className="flex overflow-x-auto px-6 gap-3 no-scrollbar pb-4 pointer-events-auto snap-x snap-mandatory">
             ...
             {buildings.map((b, i) => {
                ...
                let label = isAd ? 'Sponsored Sanctum' : `${Math.round(b.totalMinutes)} mins total`;
                let title = b.name;
                let Icon = Building;
                let iconBg = 'bg-white/10';
                let iconColor = 'text-white';
                let cardBg = 'bg-black/60 border-white/10 shadow-soft';
                let titleColor = 'text-hyve-text1';

                if (isAd) {
                  Icon = Star;
                  iconBg = 'bg-hyve-gold';
                  iconColor = 'text-black';
                  cardBg = 'bg-hyve-gold/10 border-hyve-gold/30 shadow-[0_8px_32px_rgba(201,168,106,0.15)]';
                  titleColor = 'text-hyve-gold';
                } else {
                  if (i === 0) {
                    title = "Latest Hangout";
                    label = b.name;
                    Icon = Clock;
                    iconBg = 'bg-blue-500/20';
                    iconColor = 'text-blue-400';
                    cardBg = 'bg-blue-500/5 border-blue-500/20 shadow-[0_8px_32px_rgba(59,130,246,0.1)]';
                  } else if (i === 1) {
                    title = "Most-Liked Spot";
                    label = b.name;
                    Icon = Heart;
                    iconBg = 'bg-rose-500/20';
                    iconColor = 'text-rose-400';
                    cardBg = 'bg-rose-500/5 border-rose-500/20 shadow-[0_8px_32px_rgba(244,63,94,0.1)]';
                  }
                }
                ...
                return (
                   <div 
                     ...
                     className={`snap-center shrink-0 w-52 p-2.5 rounded-3xl backdrop-blur-xl border transition-all active:scale-95 cursor-pointer flex gap-3 items-center ${cardBg}`}
                   >
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${iconBg} ${iconColor}`}>
                         <Icon size={20} fill={isAd || i === 1 ? "currentColor" : "none"} />
                      </div>
                      <div className="min-w-0 flex-1">
                         <div className="flex justify-between items-start">
                            <h3 className={`text-xs font-bold truncate mb-0.5 ${titleColor}`}>{title}</h3>
                            {isAd && <span className="text-[8px] font-black bg-hyve-gold text-black px-1.5 py-0.5 rounded-md">AD</span>}
                         </div>
                         <p className="text-[10px] text-hyve-text3 truncate">{label}</p>
