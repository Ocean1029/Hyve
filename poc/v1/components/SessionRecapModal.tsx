...
export const SessionRecapModal = ({ friend, onClose, onProfileClick }: { friend: User, onClose: () => void, onProfileClick: () => void }) => {
  const dbEntry = (HYVE_DATABASE.friendships as any)[friend.id];
  const hangout = dbEntry?.lastHangout;
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar space-y-8 relative z-10 pb-10">
           ...
           <div className="animate-flip-in" style={{ animationDelay: '0.1s' }}>
              ...
              >
                 ...
                 <div>
                    ...
                    <span className="text-[10px] text-hyve-text3 font-medium">Shared {hangout.duration}</span>
                 </div>
           ...
           <div className="w-full aspect-[4/5] rounded-[40px] overflow-hidden border border-hyve-gold/20 relative group bg-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-flip-in" style={{ animationDelay: '0.2s' }}>
              <img 
                src={`https://picsum.photos/800/1000?random=${friend.id}-session`} 
           ...
           <div>
              <div className="flex items-center gap-2 mb-1">
                 ...
                 <span className="text-xs font-bold text-hyve-text1">{hangout.date}</span>
              </div>
              <div className="flex items-center gap-2 pl-0.5">
                 <MapPin size={10} className="text-hyve-text3" />
                 <span className="text-[10px] font-medium text-hyve-text2">{hangout.location}</span>
              </div>
           </div>
