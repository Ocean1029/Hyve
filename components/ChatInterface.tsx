...
import { generateChatResponse } from '@/lib/services/geminiService';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ friend, onBack }) => {
  const [inputValue, setInputValue] = useState('');
  const [isTyping, setIsTyping] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Initial History based on actual friend interactions
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    const msgs: ChatMessage[] = [];
    
    // Check if there is a recent interaction to generate a system message
  ...
  const handleSend = async () => {
    ...
    setTimeout(async () => {
      const replyText = await generateChatResponse(friend.name, friend.bio, userText);
      
      const newFriendMsg: ChatMessage = {
      ...
      setMessages(prev => [...prev, newFriendMsg]);
      setIsTyping(false);
  ...
  return (
    <div className="absolute inset-0 z-[60] bg-zinc-950 flex flex-col animate-in slide-in-from-right duration-300">
