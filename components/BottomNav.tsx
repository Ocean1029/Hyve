...
const BottomNav: React.FC = () => {
  ...
  return (
    <div className="absolute bottom-6 left-6 right-6 z-[200] bg-zinc-900/90 backdrop-blur-xl rounded-full border border-zinc-800/50 shadow-2xl flex items-center justify-between px-2 py-2 safe-area-bottom">
       <Link 
         href="/friends"
         className={`flex-1 flex flex-col items-center justify-center gap-1 py-3 rounded-full transition-all duration-300 ${isActive('/friends') ? 'bg-zinc-800 shadow-md' : 'hover:bg-zinc-800/50'}`}
       >
         <MessageCircle className={`w-5 h-5 ${isActive('/friends') ? 'text-white fill-white' : 'text-zinc-500'}`} />
       </Link>
